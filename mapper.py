...
def parse_hour(datetime_str):
    """
    Extracts and returns a zero-padded hour string (e.g. '03', '17')
    from a datetime string in the format: M/D/YYYY H:MM:SS
 
    Args:
        datetime_str (str): Raw datetime string from the CSV field.
    Returns:
        str: Zero-padded hour string in range '00'–'23'.
    """
...
def mapper():
  ...
  logging.warning(
        f"Mapper done — processed: {lines_processed}, skipped: {lines_skipped}"
    )
